...
const sanitizeCSSValue = (value: string): string => {
  ...
  const safeColorRegex = /^(#[0-9a-fA-F]{3,6}|rgb\([0-9,\s]+\)|rgba\([0-9,.\s]+\)|hsl\([0-9,%\s]+\)|hsla\([0-9,%.s]+\)|[a-zA-Z]+)$/;
  return safeColorRegex.test(value) ? value : '';
};
...
const sanitizeCSSProperty = (property: string): string => {
  ...
  const safePropRegex = /^[a-zA-Z0-9_-]+$/;
  return safePropRegex.test(property) ? property : '';
};
