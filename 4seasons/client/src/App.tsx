...
import { useAnalytics, trackCityGuideView } from "@/lib/analytics";
...
function Router() {
  ...
  useEffect(() => {
    ...
    // Track city-specific views for better segmentation
    if (location.includes('city-guide')) {
      const cityMatch = location.match(/\/([^\/]+)-city-guide/) || location.match(/\/city-guides\/([^\/]+)/);
      if (cityMatch) {
        const cityName = cityMatch[1].replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
        trackCityGuideView(cityName);
      }
    }
  }, [location]);
...
function App() {
  ...
  // Initialize service worker, analytics, and preload critical routes on mount
  useEffect(() => {
    // Initialize analytics first (before any page tracking)
    analytics.init();
