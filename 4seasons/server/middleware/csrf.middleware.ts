...
export class CSRFProtection {
   ...
   * Verify a CSRF token
   */
  verifyCsrfTokenSignature(token: string, sessionId?: string): { valid: boolean; reason?: string } {
    if (!token) {
  ...
  verifyToken = (req: any, res: Response, next: NextFunction): void => {
    ...
    try {
      ...
      // Verify token
      const verification = this.verifyCsrfTokenSignature(providedToken, sessionId);
