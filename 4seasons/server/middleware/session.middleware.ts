...
// Initialize PostgreSQL session store as fallback
async function initializePgStore() {
  try {
    if (!process.env.DATABASE_URL) {
      logger.warn('Session: No DATABASE_URL available for PostgreSQL session store');
      return null;
    }
    ...
    logger.info('Session: PostgreSQL store initialized successfully');
    return pgStore;
  } catch (error) {
    logger.error('Session: Failed to initialize PostgreSQL store', error);
    return null;
  }
}
...
// Create session middleware with multiple store fallbacks
export async function createSessionMiddleware() {
  try {
    // Try to initialize Redis store first (preferred)
    let sessionStore = await initializeRedisStore();

    // If Redis fails, try PostgreSQL as fallback in production
    if (!sessionStore && process.env.NODE_ENV === 'production') {
      sessionStore = await initializePgStore();

      if (sessionStore) {
        logger.info('Session: Using PostgreSQL store for sessions (Redis fallback)');
      }
    }

    if (sessionStore) {
      sessionConfig.store = sessionStore;
      if (sessionStore === redisStore) {
      ...
      if (process.env.NODE_ENV === 'development') {
        ...
        // In production, we require a persistent store
        logger.error('Session: No persistent session store available in production');
        logger.error('Session: Please configure either Redis (REDIS_URL) or PostgreSQL (DATABASE_URL)');
