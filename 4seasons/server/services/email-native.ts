...
export class NativeEmailService {
  ...
  private createTransport(): void {
    ...
    this.transporter = nodemailer.createTransport({
      ...
      // GoDaddy optimizations
      connectionTimeout: isGoDaddyHosting ? 20000 : 10000, // 20s for shared hosting
      greetingTimeout: isGoDaddyHosting ? 10000 : 5000,
      socketTimeout: isGoDaddyHosting ? 30000 : 15000,
      maxConnections: isGoDaddyHosting ? 2 : 5, // Fewer connections on shared hosting
      maxMessages: isGoDaddyHosting ? 10 : 100, // Limit messages per connection
      pool: isGoDaddyHosting ? false : true, // Disable pooling on shared hosting to avoid connection issues
      // Trust GoDaddy's certificate
      tls: {
  ...
  /**
   * Test email connection with GoDaddy-specific handling
   */
  async testConnection(): Promise<boolean> {
    ...
    try {
      // Set timeout for GoDaddy hosting
      const verifyPromise = this.transporter.verify();
      const timeoutPromise = new Promise((_, reject) => {
        setTimeout(() => reject(new Error('Connection timeout')), isGoDaddyHosting ? 30000 : 10000);
      });
      ...
      logger.info('SMTP connection verified successfully', {
        host: this.config?.host,
        user: this.config?.auth.user,
        hosting: isGoDaddyHosting ? 'GoDaddy' : 'Generic'
      });
      return true;
    } catch (error: any) {
      ...
      // GoDaddy-specific error handling
      if (isGoDaddyHosting) {
        if (error.message?.includes('timeout')) {
          logger.warn('GoDaddy SMTP timeout - this is common on shared hosting');
        } else if (error.message?.includes('authentication')) {
          logger.error('GoDaddy SMTP authentication failed - check credentials');
        } else if (error.message?.includes('connection')) {
          logger.warn('GoDaddy SMTP connection issue - may be temporary');
        }
      }
