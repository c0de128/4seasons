...
class SecurityScanner {
  ...
  async scanDependencyVulnerabilities(): Promise<SecurityScanResult> {
    try {
      ...
      // Build summary using auditResult.metadata.vulnerabilities directly
      ...
      const summary = {
        ...
        total: (vulnerabilities.critical || 0) + (vulnerabilities.high || 0) + (vulnerabilities.moderate || 0) + (vulnerabilities.low || 0)
      };
