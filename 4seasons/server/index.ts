...
// MLS Test page with NO security restrictions
app.get("/mls-test", (req, res) => {
...
<head>
    ...
    <title>MLS Test Page - No Security</title>
</head>
<body style="margin: 0; padding: 0;">
    <h1 style="text-align: center; background: #0d0d33; color: white; margin: 0; padding: 10px;">MLS Test - Direct iframe (No Security)</h1>
    <iframe src="https://matrix.ntreis.net/Matrix/public/IDX.aspx?idx=2a47c86"
...
(async () => {
  ...
  server.listen(port, host, () => {
    ...
    if (isGoDaddyHosting) {
      logger.info('GoDaddy hosting optimizations enabled', {
        memoryLimit: '512MB',
        keepAliveTimeout: process.env.KEEP_ALIVE_TIMEOUT || '15000',
        requestTimeout: process.env.REQUEST_TIMEOUT || '30000',
        requestTimeoutMs: server.timeout,
